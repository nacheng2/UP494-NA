...
1+2 # This is also a comment - it is in line with some code to be interpreted
```
...
```{r}
oatmeal$instructions <- paste(oatmeal$quantity45, oatmeal$units, oatmeal$ingredients)
```
...
```{r}
?mean
mean(ozs$medhhincome2014_tract[ozs$PovertyRate > 0.2 & ozs$state == "Illinois"], na.rm=TRUE)
